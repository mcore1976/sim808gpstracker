...
uint8_t readgpsinfo()
{
  ...
  do 
  {
    ...
    } while (gpsattempts<40); // end of DO loop - only 40 attempts in 15 sec intervals to get GPS fixation - 10 minutes
...
int main(void) {
  ...
  static uint8_t bufconv[16] = "xxxxxxxxxxxxxxxx";
  uint32_t minutes, minutesdeg;
             ...
             do { 
               ...
               if (readgpsinfo()==1)
                  { 
                   ...
                   // first copy the DD degrees of LONGTITUDE                  
                   longtitudegps[0]=longtitude[0];
                   longtitudegps[1]=longtitude[1];
                   longtitudegps[2]='.';
                   longtitudegps[3]=longtitude[2];

                   // now we have to calculate DEGREES FRACTIONAL from minutes of LONGTITUDE
                   
                   // conversion from string digits to long int
                   minutes = longtitude[8] - 48;
                   minutes = minutes + ((longtitude[7] - 48) * 10);
                   minutes = minutes + ((longtitude[6] - 48) * 100);
                   minutes = minutes + ((longtitude[5] - 48) * 1000);
                   minutes = minutes + ((longtitude[3] - 48) * 10000);

                   minutesdeg = minutes / 60; 

                   ltoa(minutesdeg, bufconv,10);

                   longtitudegps[4]=bufconv[0];
                   longtitudegps[5]=bufconv[1];
                   longtitudegps[6]=bufconv[2];
                   longtitudegps[7]=bufconv[3];
                   longtitudegps[8]=bufconv[4];
                   longtitudegps[9]=0;
                    
                   // conversion of LATTITUDE NMEA to Google format
                   latitudegps[0]=latitude[0];
                   latitudegps[1]=latitude[1];
                   latitudegps[2]='.';
                   ...
                   minutes = latitude[8] - 48;
                   minutes = minutes + ((latitude[7] - 48) * 10);
                   minutes = minutes + ((latitude[6] - 48) * 100);
                   minutes = minutes + ((latitude[5] - 48) * 1000);
                   minutes = minutes + ((latitude[3] - 48) * 10000);
                   minutes = minutes + ((latitude[2] - 48) * 100000);

                   minutesdeg = minutes / 60; 

                   ltoa(minutesdeg, bufconv,10);

                   latitudegps[3]=bufconv[0];
                   latitudegps[4]=bufconv[1];
                   latitudegps[5]=bufconv[2];
                   latitudegps[6]=bufconv[3];
                   latitudegps[7]=bufconv[4];
                   latitudegps[8]=bufconv[5];
                   latitudegps[9]=0;
