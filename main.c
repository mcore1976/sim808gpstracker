...
uint8_t readgpsinfo()
{
  ...
  do 
  {
    ...
    } while (gpsattempts<20); // end of DO loop - only 10 attempts in 30 sec intervals to get GPS fixation - 5 minutes
  ...
  uint8_t initialized2, attempt2, nbrminutes;
     ...
     nbrminutes = 0;
		 ...
		 delay_sec(2);
                ...
                attempt2++; // increase number of attempts - max 48 attempts = 24 hours of no signal 2G network
