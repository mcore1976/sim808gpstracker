...
int main(void) {
  ...
  uint8_t initialized, ringrcvd,  attempt, gpsdataavailable;
             ...
             do { 
               ...
               if (readgpsinfo()==1)
                  { 
                   ...
                   // LONGTITUDE NMEA data are encoded following way : DDDMM.FFFF where 
                   // DDD = degrees 
                   // MM = minutes 
                   ...
                   // checking position of "." in LONGTITUDE OUTPUT
                   // if DD 2-digits and no MINUS sign
                   if ( (longtitude[4] == ".") && (longtitude[0] != "-" ) ) 
                      { 

                       // first copy the DD degrees of LONGTITUDE to UNSIGNED LONG VARIABLE                  
                       degrees = (longtitude[0] - '0') * 10000000UL;
                       degrees = degrees + ((longtitude[1] - '0') * 1000000UL);

                       // now we have to calculate DEGREES from minutes of LONGTITUDE
                       // and add it to DEGREES from GPS
                   
                       // conversion from string digits to long int
                       minutes = longtitude[8] - '0';
                       minutes = minutes + ((longtitude[7] - '0') * 10UL);
                       minutes = minutes + ((longtitude[6] - '0') * 100UL);
                       minutes = minutes + ((longtitude[5] - '0') * 1000UL);
                       // LONGTITUDE [4] is DOT sign in GPS output
                       minutes = minutes + ((longtitude[3] - '0') * 10000UL);
                       minutes = minutes + ((longtitude[2] - '0') * 100000UL);

                       // now conversion from minutes to degrees
                       ...
                       // degrees value is > 10M.. 48 is ASCII of '0'
                       // we are using minutesdeg variable as a buffer for calculation
                       longtitudegps[0] = '0' + (degrees / 10000000UL);
                       longtitudegps[1] = '0' + ( (degrees % 10000000UL) / 1000000UL );
                       longtitudegps[2] = '.';

                       // we are using minutesdeg as a buffer for calculations
                       /*
                       minutesdeg = (degrees % 10000000UL) % 1000000UL;
                       longtitudegps[3] = '0' + ( minutesdeg / 100000UL );
                       minutesdeg = minutesdeg % 100000UL;
                       longtitudegps[4] = '0' + ( minutesdeg / 10000UL );
                       ...
                       */
                       ...
                       minutesdeg = minutesdeg % 100000UL;
                       minutesdeg = minutesdeg % 10000;
                       longtitudegps[3] = '0' + ( minutesdeg / 1000UL );
                       minutesdeg = minutesdeg % 1000;
                       longtitudegps[4] = '0' + ( minutesdeg / 100UL );
                       minutesdeg = minutesdeg % 100;
                       longtitudegps[5] = '0' + ( minutesdeg / 10UL );
                       minutesdeg = minutesdeg % 10;
                       longtitudegps[6] = '0' + ( minutesdeg / 1UL );
                        // put null at the end of string
                       longtitudegps[7] = 0x00;
                      }


                   // checking position of "." in LONGTITUDE OUTPUT
                   // if DD 3-digits and no MINUS sign
                   if ( (longtitude[5] == ".") && (longtitude[0] != "-" ) ) 
                      { 

                       // first copy the DD degrees of LONGTITUDE to UNSIGNED LONG VARIABLE                  
                       degrees = (longtitude[0] - '0') * 100000000UL;
                       degrees = degrees + ((longtitude[1] - '0') * 10000000UL);
                       degrees = degrees + ((longtitude[2] - '0') * 1000000UL);

                       // now we have to calculate DEGREES from minutes of LONGTITUDE
                       // and add it to DEGREES from GPS
                   
                       // conversion from string digits to long int
                       minutes = longtitude[9] - '0';
                       minutes = minutes + ((longtitude[8] - '0') * 10UL);
                       minutes = minutes + ((longtitude[7] - '0') * 100UL);
                       minutes = minutes + ((longtitude[6] - '0') * 1000UL);
                       // LONGTITUDE [4] is DOT sign in GPS output
                       minutes = minutes + ((longtitude[4] - '0') * 10000UL);
                       minutes = minutes + ((longtitude[3] - '0') * 100000UL);
                       ...
                       // now we have to copy the output to text in Google format
                       // degrees value is > 100M.. 48 is ASCII of '0'
                       // we are using minutesdeg variable as a buffer for calculation
                       longtitudegps[0] = '0' + (degrees / 100000000UL);
                       minutesdeg = degrees % 100000000UL;
                       longtitudegps[1] = '0' + ( minutesdeg / 10000000UL );
                       longtitudegps[2] = '0' + ( ( minutesdeg % 10000000UL) / 1000000UL );
                       longtitudegps[3] = '.';
                       ...
                       minutesdeg = minutesdeg % 100000UL;
                       minutesdeg = minutesdeg % 10000;
                       longtitudegps[4] = '0' + ( minutesdeg / 1000UL );
                       minutesdeg = minutesdeg % 1000;
                       longtitudegps[5] = '0' + ( minutesdeg / 100UL );
                       minutesdeg = minutesdeg % 100;
                       longtitudegps[6] = '0' + ( minutesdeg / 10UL );
                       minutesdeg = minutesdeg % 10;
                       longtitudegps[7] = '0' + ( minutesdeg / 1UL );
                        // put null at the end of string
                       longtitudegps[8] = 0x00;
                      }
                   ...
                   // LATTITUDE NMEA data are encoded following way : DDMM.FFFF where 
                   // DD = degrees 
                   // MM = minutes 
                   ...
                   // checking if lattitude is 2 digits and NOT below zero
                   if ( (latitude[4] == ".") && (latitude[0] != "-" ) ) 
                      { 
                      // first copy the DD degrees of LATITUDE to UNSIGNED LONG VARIABLE                  
                      degrees = (latitude[0] - '0') * 10000000UL;
                      degrees = degrees + ((latitude[1] - '0') * 1000000UL);

                      // now we have to calculate DEGREES from minutes of LATITUDE
                      // and add it to DEGREES from GPS
                   
                      // conversion from string digits to long int
                      minutes = latitude[8] - '0';
                      minutes = minutes + ((latitude[7] - '0') * 10UL);
                      minutes = minutes + ((latitude[6] - '0') * 100UL);
                      minutes = minutes + ((latitude[5] - '0') * 1000UL);
                      // LATITUDE [4] is DOT sign in GPS output
                      minutes = minutes + ((latitude[3] - '0') * 10000UL);
                      minutes = minutes + ((latitude[2] - '0') * 100000UL);
                      ...
                      // degrees value is > 10M.. 48 is ASCII of '0'
                      // we are using minutesdeg variable as a buffer for calculation
                      latitudegps[0] = '0' + (degrees / 10000000UL);
                      latitudegps[1] = '0' + ( (degrees % 10000000UL) / 1000000UL );
                      latitudegps[2] = '.';

                      // we are using minutesdeg as a buffer for calculations
                      /*
                      minutesdeg = (degrees % 10000000UL) % 1000000UL;
                      latitudegps[3] = '0' + ( minutesdeg / 100000UL );
                      minutesdeg = minutesdeg % 100000UL;
                      ...
                      // putt NULL at the end of string
                      latitudegps[9] = 0x00;
                      */
                      // above is correct but somehow  I had to modify it to this...
                      minutesdeg = (degrees % 10000000UL) % 1000000UL;
                      minutesdeg = minutesdeg % 100000UL;
                      minutesdeg = minutesdeg % 10000UL;
                      latitudegps[3] = '0' + ( minutesdeg / 1000UL );
                      minutesdeg = minutesdeg % 1000UL;
                      latitudegps[4] = '0' + ( minutesdeg / 100UL );
                      minutesdeg = minutesdeg % 100UL;
                      latitudegps[5] = '0' + ( minutesdeg / 10UL );
                      minutesdeg = minutesdeg % 10UL;
                      latitudegps[6] = '0' + ( minutesdeg / 1UL );
                      // putt NULL at the end of string
                      latitudegps[7] = 0x00;
                      };
                   ...
                   // checking if lattitude is 1 digit and NOT below zero
                   if ( (latitude[3] == ".") && (latitude[0] != "-" ) ) 
                      { 
                      // first copy the D degrees of LATITUDE to UNSIGNED LONG VARIABLE                  
                      degrees = (latitude[0] - '0') * 100000UL;

                      // now we have to calculate DEGREES from minutes of LATITUDE
                      // and add it to DEGREES from GPS
                   
                      // conversion from string digits to long int
                      minutes = latitude[6] - '0';
                      minutes = minutes + ((latitude[5] - '0') * 10UL);
                      minutes = minutes + ((latitude[4] - '0') * 100UL);
                      // LATITUDE [4] is DOT sign in GPS output
                      minutes = minutes + ((latitude[2] - '0') * 1000UL);
                      minutes = minutes + ((latitude[1] - '0') * 10000UL);
                      ...
                      // degrees value is > 10M.. 48 is ASCII of '0'
                      // we are using minutesdeg variable as a buffer for calculation
                      latitudegps[0] = '0' + (degrees / 100000UL);
                      latitudegps[1] = '.';

                      // we are using minutesdeg as a buffer for calculations
                      minutesdeg = degrees % 100000UL;
                      minutesdeg = minutesdeg % 10000UL;
                      minutesdeg = minutesdeg % 1000UL;
                      latitudegps[2] = '0' + ( minutesdeg / 100UL );
                      minutesdeg = minutesdeg % 100UL;
                      latitudegps[3] = '0' + ( minutesdeg / 10UL );
                      minutesdeg = minutesdeg % 10UL;
                      latitudegps[4] = '0' +  minutesdeg;
                      // putt NULL at the end of string
                      latitudegps[5] = 0x00;
                      };


                   // checking if lattitude is 2 digits and BELOW ZERO
                   if ( (latitude[5] == ".") && (latitude[0] == "-" ) ) 
                      { 
                      // first copy the DD degrees of LATITUDE to UNSIGNED LONG VARIABLE
                      // bypassing latitude[0] which is '-' sign..                  
                      degrees = (latitude[1] - '0') * 10000000UL;
                      degrees = degrees + ((latitude[2] - '0') * 1000000UL);

                      // now we have to calculate DEGREES from minutes of LATITUDE
                      // and add it to DEGREES from GPS
                   
                      // conversion from string digits to long int
                      minutes = latitude[9] - '0';
                      minutes = minutes + ((latitude[8] - '0') * 10UL);
                      minutes = minutes + ((latitude[7] - '0') * 100UL);
                      minutes = minutes + ((latitude[6] - '0') * 1000UL);
                      // LATITUDE [4] is DOT sign in GPS output
                      minutes = minutes + ((latitude[4] - '0') * 10000UL);
                      minutes = minutes + ((latitude[3] - '0') * 100000UL);
                      ...
                      latitudegps[2] = '0' + ( (degrees % 10000000UL) / 1000000UL );
                      latitudegps[3] = '.';
                   ...
                   // checking if lattitude is 1 digit and  below zero
                   if ( (latitude[4] == ".") && (latitude[0] == "-" ) ) 
                      { 
                      // first copy the D degrees of LATITUDE to UNSIGNED LONG VARIABLE                  
                      degrees = (latitude[1] - '0') * 100000UL;
                      ...
                      minutes = minutes + ((latitude[6] - '0') * 10UL);
                      minutes = minutes + ((latitude[5] - '0') * 100UL);
                      // LATITUDE [4] is DOT sign in GPS output
                      minutes = minutes + ((latitude[3] - '0') * 1000UL);
                      minutes = minutes + ((latitude[2] - '0') * 10000UL);
                      ...
                      // we are using minutesdeg variable as a buffer for calculation
                      latitudegps[0] = '-';
                      latitudegps[1] = '0' + (degrees / 100000UL);
                      latitudegps[2] = '.';
                      ...
                      minutesdeg = minutesdeg % 10000UL;
                      minutesdeg = minutesdeg % 1000UL;
                      latitudegps[3] = '0' + ( minutesdeg / 100UL );
                      minutesdeg = minutesdeg % 100UL;
                      latitudegps[4] = '0' + ( minutesdeg / 10UL );
                      minutesdeg = minutesdeg % 10UL;
                      latitudegps[5] = '0' +  minutesdeg;
                      // putt NULL at the end of string
                      latitudegps[6] = 0x00;
                      };
