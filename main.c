 ...
 * baudrate UART is 9600 as most stable using RC internal clock (8MHz no division)
 *
...
// Sleepmode ON OFF - mode #1 requires DTR pin manipulation, to get out of SIM808 sleepmode DTR must be LOW for 50 miliseconds
...
const char GPSISFIXED[] PROGMEM = {"+CGPSSTATUS: Location 3D Fix"};  // GPS in now fixed Response code
...
uint8_t readgpsinfo()
{
  uint8_t gpsattempts;   // counter on attempts to get proper GPS position from SIM808 - for indoor scenarios
  gpsattempts = 0;
  ...
  // now we have to wait some time until SIM808 gets GPS signal, assume we are waiting 3 minutes and then quit
  do 
  {
    ...
    } while (gpsattempts<9); // end of DO loop - only 9 attempts in 20 sec intervals to get GPS fixation
     ...
     initialized2 = 0;
		 ...
		 delay_sec(2);
                 ...
                 if (readline()>0)
                   ...
                   if (initialized2 == 0)
                     {  
                       // if not registered or something wrong wait 4 minutes in flight mode
                       delay_sec(1);           
       ...
       while (1) {
			...
			asm volatile (
			    ...
			    "    brne 1b"	"\n"
                        ...
                        delay_sec(1);
                        uart_puts_P(SLEEPOFF);  // switch off to SLEEPMODE = 0
                        PORTC |= _BV(PC5);      // Toggles again HIGH the DTR pin - default state
                        delay_sec(1);
