          ...
          if (readline()>0)       // check GPS status response if NOT FIXED
           {
                    ...
                    if (is_in_rx_buffer(response, buf ) == 1) 
                          {
                            // we are waiting for at least 6 sattelites for good result
                            if((atoi(numsattelites))>5) gpsfixed = 1;
                            // if we are in searching loop also quit
                            if (gpsattempts == 39)  gpsfixed = 1;  
             ...
             do { 
               ...
               if (readgpsinfo()==1)
                  { 
                   ...
                   gpsdataavailable = 1; 
                   // conversion of NMEA GPS to GOOGLE GPS ( floats longtitude/latitude have to be divided by 100 )
                   // so we simply move DOT to proper location without any special math...
                   // this is division by 100 of LONGTITUDE NMEA to Google format
                   longtitudegps[0]=longtitude[0];
                   longtitudegps[1]=longtitude[1];
                   longtitudegps[2]='.';
                   longtitudegps[3]=longtitude[2];
                   longtitudegps[4]=longtitude[3];
                   longtitudegps[5]=longtitude[5];
                   longtitudegps[6]=longtitude[6];
                   longtitudegps[7]=longtitude[7];
                   longtitudegps[8]=longtitude[8];
                   longtitudegps[9]=0;

                   // this is division by 100 of LATTITUDE NMEA to Google format
                   latitudegps[0]=latitude[0];
                   latitudegps[1]=latitude[1];
                   latitudegps[2]='.';
                   latitudegps[3]=latitude[2];
                   latitudegps[4]=latitude[3];
                   latitudegps[5]=latitude[5];
                   latitudegps[6]=latitude[6];
                   latitudegps[7]=latitude[7];
                   latitudegps[8]=latitude[8];
                   latitudegps[9]=0;
                   }
