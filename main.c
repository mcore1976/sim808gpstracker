 ...
 * other info :
 * the prototype board was built using www.and-global.com 
...
// ----------------------------------------------------------------------------------------
// retrieve position from GPS and put it to LOC and LATT buffers 
// ----------------------------------------------------------------------------------------
uint8_t readgpsinfo()
{
  uint8_t gpsattempts;
  ...
  do 
  {
    ...
    } while (gpsattempts<9); // end of DO loop - only 3 attempts to get GPS fixation
...
return(0); // 0 means that GPS was unable to fix to sattelite
}
       ...
       while (1) {
			...
			asm volatile (
			    ...
			    "    brne 1b"	"\n"
                        ...
                        uart_puts_P(DELSMS);
                        delay_sec(3);
                        initialized = 1; // mark 'initialized' to further proceed outside do-while loop
                      } 
			...
			asm volatile (
               ...
               // if checking SIM808 GPS was successful mark it 
               if (readgpsinfo()==1) 
                  { 
                   gpsdataavailable = 1; 
                   }
	        ...
	        uart_puts_P(CRLF);   			   
         	...
         	// put info about DATE,TIME, LONG, LATT
                if (  gpsdataavailable == 1)  uart_puts(response);  // send Date & Time info from AGPS cell info  
