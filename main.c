...
uint8_t readgpsinfo()
{
  ...
  // now we have to wait some time until SIM808 gets GPS signal, assume we are waiting 5 minutes and then quit
  do 
  {
    ...
    } while (gpsattempts<10); // end of DO loop - only 10 attempts in 30 sec intervals to get GPS fixation - 5 minutes
                   ...
                   if (initialized2 == 0)
                    ...
                    // enter SLEEP MODE of SIM800L for power saving when no coverage 
                     ...
                     // now wait XX min before turning on radio again, here XX = 30 min
                       for (nbrminutes = 0; nbrminutes<2; nbrminutes++) 
                          { 
                          delay_sec(60); 
                          };  
