...
const char CARBATTREAD[] PROGMEM =  {"Car Battery voltage reading [mV] = "};    // Car battery reading
...
uint16_t checkcarbattery()
{
	...
	accvolt = (((uint32_t)accvolt * 3300UL) >> 10); 
                                 ...
                                 {    // there was NOTHING received over serial port so we are still waiting...
                                     ...
                                     if (nbr50useconds == 18000000UL)
                                          { // if specified amount of second passed, we need to check 2G network coverage
                                            ...
                                            // Now make ADC readings and compare if its safe
                                            // check if CAR BATTERY is within defined limits
                                            accvolt = 0;
                                            checkcarbattery(accvolt);
                                            if ( (accvolt < 1643) || (accvolt > 2497) )
                                                { 
                                                 // Now convert Integer to String to send SMS
                                                 itoa( accvolt, battery, 10);  
                                                 ...
                                                 if (accvolt > 2497 ) uart_puts_P(CARBATTOVER);
                                                 if ( (accvolt < 1643) && (accvolt > 1111) ) uart_puts_P(CARBATTUNDER);
                                                 if ( accvolt < 1111 )  uart_puts_P(CARBATTDISC);
