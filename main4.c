 ...
 *
 * this version has more accurate positioning from GPS module
 * ----------------------------------------------------------------------------------------------
 */
...
const char ISTRACK[] PROGMEM = {"TRACK"};                      // TRACK option gives 5 continous measurements
...
uint8_t readcellgps()
{
  ...
  // i counter of received chars... not to get deadlock. 70 chars max as a safe fuse 
   i = 0;
      ...
      // if 'i' fuse limit reached return with 0, probably module restarted itself
          if (i == 70) return(0);
...
uint8_t readsim808gps()
{
  ...
  // set positions to start of each buffer
   ...
   uart_puts_P(GPSINFO);      // try to retrieve GPS position
      ...
      // check if deadlocked and buffer overrun
         if (i == 150) return(0);
    ...
    while (1) {
        ...
        do { 
             ...
             // If RI/RING is unavailable then we are waiting for something from uart
                ...
                if (readline()>0)
                    ...
                    //  if SMS "TRACK" was received - we will do continous GPS measurements - 5 results will be send
                    ...
                    if  ( (is_in_rx_buffer(response, buf) == 1) && (ringrcvd == 0) )  
                            { 
                                 ...
                                 // checking if there is "TRACK" word in SMS content buffer
                                 memcpy_P(buf, ISTRACK, sizeof(ISTRACK));  
        ...
        do  {
               ...
               // to do it we have to connect to GPRS and querry Google with AT+CIPGSMLOC command
                 ...
                 // if CELL GPS data availbale or real SIM808 GPS data available then proceed with sending SMS                   
                 // avoid sending SMS with no usable information
 
                if ( (cellgpsavailable == 1) || ( gpsdataavailable == 1) )
                   {
                          // check battery voltage
                          delay_sec(1);
                   ...
                   };   // end of IF for gpsavialable and cellgps available
