...
uint8_t readgpsinfo()
{
  ...
  {
          if ( continousgps == 255 ) 
             {  delay_sec(1);  }           // if GUARD mode - do not wait so long
          else 
             { delay_sec(15);  };          // in NOT in GUARD mode - wait 15 sec for first fix 
...
int main(void) {
  ...
  // delay another 30 sec to search for GSM network
  delay_sec(30);
        ...
        do  {
               ...
               if ( (gpsdataavailable == 0) && (continousgps != 255)  )
                ...
                // if ending sequence of GPS checking simply wait until SMS is delivered
                   if ( continousgps == 1)   delay_sec(10);  
                ...
                // if GUARD MODE is active check if incoming STOP text message
                   if ( continousgps == 255)     
                          {   // wait 1 minute for SMS to stop GUARD MODE
                                ...
                                while( (initialized == 0) || (nbr50useconds < 1200000UL) )
                                    {
                                     if ( UCSR0A & (1<<RXC0)) 
                                         { initialized = 1;    }          // if something on serial port quit waiting
                                     else 
                                         { nbr50useconds++;               // increase number of 50useconds waited
                                           delay_50usec(1UL);  };         // wait another 50usec
                                    };   // end of WHILE for SMS and delaying 1 sec

                                if ( initialized == 1 )      // if SMS arrived we need to check it
                                   {  
