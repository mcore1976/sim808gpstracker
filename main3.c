...
// ----------------------------------------------------------------------------------------------------
// read SMS message PHONE NUMBER from +CMT: output and copy it to buffer 'phonenumber' for SMS sending
// ----------------------------------------------------------------------------------------------------
...
int main(void) {
  ...
  uint8_t initialized, ringrcvd,  attempt, gpsdataavailable, latitudelen, longtitudelen, continousgps;
  ...
  // neverending LOOP
       ...
       while (1) {
             ...
             do { 
                ...
                // read phone number of incoming SMS, will be needed for sending 
                   uart_puts_P(SMS1);
                   delay_sec(2); 
                   uart_puts_P(SHOWSMS); 
                   delay_sec(2);
                ...
                if (readline()>0)
                  {
                    ...
                    if  ( (is_in_rx_buffer(response, buf) == 1) && (ringrcvd == 0) )  
                     ...
                     // else if some SMS was received - we will do continous GPS measurements - 5 results will be send
                    ...
                    if  ( (is_in_rx_buffer(response, buf) == 1) && (ringrcvd == 0) )  
                     ...
                     // if some other message than "RING" or "ING" or SMS check if network is avaialble and SIM808 is operational  
           ...
           do  {
               ...
               // to do it we have to connect to GPRS and querry Google with AT+CIPGSMLOC command
                ...
                // decrease continousgps attempt number
                   continousgps--;
                ...
                // just in case SMS confirmation was received not to trigger anything
                   if (continousgps > 0)
                        {  
                          // if in GPS continous mode delay 3 minutes before next attempt
                          delay_sec(60);
                          delay_sec(60);
                          delay_sec(60);
                         }
