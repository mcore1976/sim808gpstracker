...
uint8_t readcellgps()
{
  ...
  // i counter of received chars... not to get deadlock. 70 chars for full message max as a safe fuse 
   i = 0;
  ...
  uint8_t initialized2, attempt2, nbrminutes;
     ...
     // if not registered enable radio.. just in case
              ...
              do { 
                 ...
                 delay_sec(120);           // first searching 2 min just in case some instability
                      ...
                      if (initialized == 1)
                                    {
                                     // GET CELL ID OF BASE STATION and query Google for coordinates then send over SMS with google map loc
                                      delay_sec(1);
                                     // attention some SIM808 boards will restart after this command.... FW issues
