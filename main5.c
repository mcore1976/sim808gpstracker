 ...
 * this version is suitable for the SIM808 boards that do not have DTR and RI ping exposed
 *  
...
const char COMMANDNOK[] PROGMEM = {"WRONG COMMAND\n"};                  // Information that teh SMS command was wrong
const char ACTIVATED[] PROGMEM =  {"ACTIVATED CALLS FROM "};            // Ack activated sending number as allowed to call 
const char GUARD[] PROGMEM =      {"GUARD MODE ACTIVATED "};            // Confirmation of guard mode activated 
...
volatile static uint8_t latitudegpsold[20] = "12345678901234567890";
volatile static uint8_t longtitudegpsold[20] = "12345678901234567890";
  ...
  uint8_t initialized2, attempt2, nbrminutes;
     ...
     // if not registered enable radio.. just in case
              ...
              do { 
                 ...
                 delay_sec(120);          // first searching 2 min in case of instability
...
int main(void) {
  ...
  uint8_t initialized, ringrcvd,  attempt, cellgpsavailable, gpsdataavailable, latitudelen, longtitudelen;
                ...
                if (readline()>0)
                    ...
                    if  ( (is_in_rx_buffer(response, buf) == 1) && (ringrcvd == 0) )  
                            { 
                                 ...
                                 if   (is_in_rx_buffer(smstext, buf) == 1)  
                                     {
                                        // disable SLEEPMODE 
                                        //PORTC &= ~_BV(PC5);  // Toggle LOW the DTR pin of SIM808
                                        // send first dummy AT command
                                 ...
                                 if   (is_in_rx_buffer(smstext, buf) == 1)  
                                     {
                                       ...
                                       // copy SMS sender phone to buffer for further comparision
                                        memcpy(smsphonenumber, phonenumber, sizeof(phonenumber));  
                                       ...
                                       // send a SMS configrmation of the command
                                        ...
                                        // compose an SMS from fragments - interactive mode CTRL Z at the end
                                        uart_puts_P(SMS2);
                                        uart_puts(smsphonenumber);  // send phone number received from CLIP
                                        uart_puts_P(CRLF);                                       
                                        delay_sec(1); 
                                 ...
                                 if   (is_in_rx_buffer(smstext, buf) == 1)  
                                    {
                                     ...
                                     // some other SMS content, we need to respond with "wrong command"
                                if ((ringrcvd == 0) && (initialized ==0))    
                                     {
                                        // disable SLEEPMODE 
                                        //PORTC &= ~_BV(PC5);  // Toggle LOW the DTR pin of SIM808
                                        // send first dummy AT command
                                       ...
                                       // send a SMS configrmation of the command
                                        ...
                                        uart_puts_P(CRLF);                                       
                                        delay_sec(1); 
                                        uart_puts_P(COMMANDNOK);
                                       // end the SMS message
                                        send_uart(26);   // ctrl Z to end SMS
                                        delay_sec(10); 


                                     // there was something different than expected SMS content so we need to go back to the beginning - clear the flag
                                        initialized = 0;
                                        ringrcvd = 0; 
                                        continousgps = 0; 
                                     };
                     ...
                     if (is_in_rx_buffer(response, buf) == 1) 
                           { 
                                ...
                                if (is_in_rx_buffer(phonenumber, smsphonenumber) == 1) 
                                    {
                                     // mark 'initialized' flag to further proceed outside do-while loop
                                     initialized = 1;
                                     ringrcvd = 1; 
        ...
        do  {
               ...
               if (readgpsinfo()==1)
                  { 
                   ...
                   // DDD + (MMFFFFF/60) for both LATT and LONG
                   ...
                   // LATTITUDE NMEA data are encoded following way : DDMM.FFFFFF where 
                   // DD = degrees 
                   // MM = minutes 
               ...
               if ( (readgpsinfo()==0) && (continousgps != 255)  )
                  {   
                ...
                if (  ( (cellgpsavailable == 1) || ( gpsdataavailable == 1) ) && (continousgps != 255) )
                 ...
                 // if CELL GPS data availbale or real SIM808 GPS data available and GUARD MODE enabled
                ...
                if (  ( gpsdataavailable == 1) && (continousgps == 255) )
                   {
                    ...
                    if ( (strncmp(latitudegpsold, latitudegps, (sizeof(latitudegps)-2) )!=0 ) ||  (strncmp(longtitudegpsold, longtitudegps, (sizeof(longtitudegps)-2))!=0 ) ) 
                        ...
                        // clear continousgps flag by setting to 0 - get out of GUARD MODE
                          continousgps = 0;      
                          delay_sec(10);
                        ...
                        {
                        // else simply wait 1 minute before next GPS check
                          delay_sec(60);
                        };   // end of ELSE 
                   };

                // copy current GPS position as old GPS position for comparision during GUARD mode
                   memcpy(latitudegpsold, latitudegps, sizeof(latitudegps));  
                   memcpy(longtitudegpsold, longtitudegps, sizeof(longtitudegps));  
                ...
                // just in case SMS confirmation was received not to trigger anything
                   if ( (continousgps > 0) && (continousgps<255) )
                        ...
                        {  
                          // otherwise simply wait until SMS is delivered
                          delay_sec(10); 
                        };
